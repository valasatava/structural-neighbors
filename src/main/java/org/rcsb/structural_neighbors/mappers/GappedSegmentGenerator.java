...
/**
 * A mapper from {@link StructureDataInterface} to the {@link Point3d}[] of the
 * calpha coordinates.
 * 
 ...
 *
 */
public class GappedSegmentGenerator
		implements PairFlatMapFunction<Tuple2<String, StructureDataInterface>, String, WritableSegment> {
	 ...
	 *            the length of each fragment. Null means take each Chain as as
	 *            single fragment.
	 */
	...
	public Iterable<Tuple2<String, WritableSegment>> call(Tuple2<String, StructureDataInterface> t) throws Exception {
		...
		for (int i = 0; i < structureDataInterface.getNumChains(); i++) {
			...
			if (polymer[i]) {
				...
				if ( ( (float) StringUtils.countMatches(sequence, "X"))/sequence.length() > 0.5) {
					polymer[i] = false;
				}
	 ...
	 *            the input {@link StructureDataInterface}
	 ...
	 * 
	 * @param structureDataInterface
	 *            the {@link StructureDataInterface} to read
	 * @param groupType
	 *            the integer specifying the grouptype
	 * @param atomCounter
	 *            the atom count at the start of this group
